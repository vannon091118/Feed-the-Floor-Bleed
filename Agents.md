...
Diese Datei ist die einzige kanonische Agent-Governance für das Repository. Weitere eigenständige Agent-Dokumente werden nicht geführt; Erkenntnisse gehören hierher.
...
packages/
  ...
  server/
    ...
    src/sync/         # Upload/Results, D1-Jobstatus, Queues, Replay
scripts/
...
| **Gesamtes Quellcode-Budget** | **kein Gesamt-Cap** | Repository-Größe darf nicht künstlich begrenzt werden |
| **Globaler Datei-Cap** | **dynamisch** | Globaler LOC-Gate übernimmt den höchsten konfigurierten Ownership-Cap |

- **Globaler LOC-Gate:** Jede Datei unter `packages/` und `scripts/` muss den dynamisch aus `policy.json` abgeleiteten Datei-Cap einhalten. Es gibt bewusst **kein künstliches Repository-Gesamtbudget**; die tatsächliche Gesamtgröße wird nur zur Transparenz ausgegeben.
...
|--------|--------------|-------|
| `loc-gate` | **immer** | Ownership-LOC-Caps pro Datei (Kommentare ignoriert) |
| `global-loc-gate` | **immer** | Globaler Datei-Cap; Gesamtgröße wird nur berichtet, nicht künstlich begrenzt |
...
| `redundancy-gate` | **immer** | Duplizierte Sechs-Zeilen-Blöcke in Package-Quellen |
...
| `schema-contract` | **Base immer** | Zod-Contracts, `sim_version` und Protokoll-Keys |
